classdef arcfaceLayer < nnet.layer.Layer
    ...
    %                arcfaceLayer(),... % 还需要连接一个label标签层，目前版本并不支持该特性
    %                softmaxLayer(),...
    %                classificationLayer()]
    %
    % layer = arcfaceLayer();
    ...
    methods
        function layer = arcfaceLayer(numInputs,name,embedding_size, classnum,  s, m) 
            % init function
            arguments
                numInputs (1,1) double = 2
            ...
            layer.NumInputs = numInputs; % Define Custom Deep Learning Layer with Multiple Inputs
        ...
        function Z = predict(layer, varargin)
            ...
            %     输入形式h*w*c*bs
            %     Labels， 1*bs, 标签从1开始
            %     或onehotLabel, 必须为1*1*classnum*bs
            %     四维矩阵，第3维（channel）只有一个1，其余都为0，要跳过checklayer检查，因为checkLayer会自动赋[-1,1]的值检查
            % 输出：
            ...
            %
            X = varargin{1};
            Labels = varargin{2};% bs*classnum二维矩阵
%             onehotLabel =  varargin{2};
            
            % get batch size and onehot label
            bs = size(X,4);
            onehotLabel = zeros(bs,layer.ClassNum,'like',Labels); % bs*classnum二维矩阵，每行只有一个1
            for i = 1:bs
                onehotLabel(i,Labels(i)) = 1;% 要跳过checklayer检查，因为checkLayer会自动赋[-1,1]的值检查，Labels(i)在区域内无法索引
            end
            ...
            output = cos_theta;
%             onehotLabel = squeeze(onehotLabel);% classnum*bs
%             onehotLabel = onehotLabel';% bs*classnum
            output(onehotLabel) = cos_theta_m(onehotLabel); % bs*classnum, only change the correct predicted output
